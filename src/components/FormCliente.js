...
const FormCliente = ({ clienteAtual, onSave }) => {
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <label>
        Observações:
        <textarea name="observacoes" value={cliente.observacoes} onChange={handleChange}></textarea>
      </label>
      <label>
        Ativo:
        <input type="checkbox" name="ativo" checked={cliente.ativo} onChange={() => setCliente({ ...cliente, ativo: !cliente.ativo })} />
      </label>
